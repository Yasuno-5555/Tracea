use crate::emitter::traits::{UnifiedOpIR, UnifiedOpType, EmissionError};
...
pub fn generate_conv(ir: &UnifiedOpIR) -> Result<String, EmissionError> {
...
"#, 
        ...
        Err(EmissionError::UnsupportedOpType {
            reason: "Using conv emitter for non-conv op".to_string(),
        })
    }
}
