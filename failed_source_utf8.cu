...
__device__ __forceinline__ void mbarrier_wait(uint64_t* mbarrier_ptr, uint64_t phase) {
    ...
    asm volatile(
        ...
        "  while_not_ready_label:\n\t"
        "    @!p mbarrier.test_wait.shared.b64 p, [%0], %1;\n\t"
        "    @!p bra while_not_ready_label;\n\t"
        "}\n\t"
        : 
...
extern "C" __global__ void __launch_bounds__(NUM_WARPS * 32, 1) gemm_mma_kernel(
    ...
    half* sA = (half*)(smem);
    half* sB = (half*)(smem + 8192);

    int a_tile_row = blockIdx.y * MT;
    int b_tile_col = blockIdx.x * NT;
    
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> frag_acc[MT/128][NT/16]; // 1x8 frags for 128x128 with 8 warps
    #pragma unroll
    for(int mi=0; mi<MT/128; mi++)
        for(int ni=0; ni<NT/16; ni++)
            wmma::fill_fragment(frag_acc[mi][ni], (half)0.0f);

        // Compute Stage Baseline (No Partitioning)
        int mt_per_warp = MT / 8;
        const int M_FRAGS = 1; // 128 / 8 / 16 = 1
        const int N_FRAGS = NT / 16;
        int cons_warp = warp_id;

        for (int k_tile = 0; k_tile < (K + KT - 1) / KT; ++k_tile) {
            // Load Stage
            #pragma unroll
            for (int i = tid; i < (MT * KT) / 8; i += 32 * 8) {
                int m = (i * 8) / KT;
                int k = (i * 8) % KT;
            ...
            #pragma unroll
            for (int i = tid; i < (KT * NT) / 8; i += 32 * 8) {
                int k = (i * 8) / NT;
                int n = (i * 8) % NT;
